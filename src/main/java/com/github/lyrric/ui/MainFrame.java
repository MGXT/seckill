        ...
        new Thread(() -> {
            do {
                try {
                    ...
                    long serverTime = service.httpService.getServerTime();
                    long end = System.currentTimeMillis();
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS");
                    SecDate.secGap = serverTime - start;
                    note.append("系统调用开始时间：" + sdf.format(new Date(start)) + "\r\n");
                    note.append("系统调用结束时间：" + sdf.format(new Date(end)) + "\r\n");
                    note.append("当前服务时间：" + sdf.format(new Date(serverTime)) + "\r\n");
                    note.append("时间差：" + (serverTime - start) + "\r\n");
                    note.append("-------------------------\r\n");
                    Thread.currentThread().join(5000);
                } catch (Exception e) {
                    note.append("服务器时间获取错误\r\n");
                }
            } while (true);
        }).start();
